...
selected_book = st.sidebar.selectbox(
    ...
    ["draftkings", "fanduel", "betmgm", "caesars", "pointsbet", "consensus"]
)
...
if player_count == 0:
    ...
    for idx, (_, p_row) in enumerate(unique_players.iterrows()):
        ...
        for stat in stats_to_analyze:
            ...
            # --- DYNAMIC VENDOR STRIPPING ENGINE ---
            # Evaluates the live target bookmaker, then falls back to consensus if empty
            for api_prop in api_props_list:
                ...
                if p_name.lower() in api_player.lower() and api_type == stat:
                    # Match selected book first; if not found, grab the first available book as consensus
                    if selected_book in api_vendor or not real_line_found:
                        ...
                        if selected_book in api_vendor:
                            break  # Perfect match located, break out of loop
            
            # Algorithmic backup line generation if no lines are open on the board yet
...
if results_list:
    ...
    st.subheader(f"🔥 Top 20 Best Value Props ({target_book.upper()} Focus): {selected_matchup}")
    st.dataframe(
        board_df[[
        ...
        if pd.isna(row['outcome']) or row['outcome'] in ["Pending", "⌛ Pending Score"]:
            ...
            if not actual_game_log.empty:
                ...
                if pd.notna(actual_val):
                    ...
                    if actual_val == line:
                        outcome_str = "⚖️ PUSH"
                    elif (is_over_rec jogging_val and actual_val > line) or (not is_over_rec and actual_val < line):
                        outcome_str = "🎯 HIT"
                    else:
                        outcome_str = "❌ MISS"
