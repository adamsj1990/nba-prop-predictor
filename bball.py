...
# Fallback block to load historical backups if calendar returns a blank slate
...
def american_to_probability(odds):
    """Converts standard American moneyline odds into a true probability rate percentage."""
    if odds is None or odds == 0:
        return 50.0  # Safe default if odds are missing
    if odds < 0:
...
# --- ROSTER FILTER (STRICT 14-DAY RECENCY TRACKING) ---
...
if player_count == 0:
    ...
    for idx, (_, p_row) in enumerate(unique_players.iterrows()):
        ...
        for stat in stats_to_analyze:
            ...
            # --- OVERHAUL: LOOK FOR LIVE SPORTSBOOK LINES VIA API PAYLOAD ---
            real_line_found = False
            vegas_line = None
            over_odds = None
            under_odds = None
            
            # Scan API data blocks for matching player metrics
            for api_prop in api_props_list:
                ...
                if p_name.lower() in api_player.lower() and api_type == stat:
                    # Consensus filter: take standard market vendor details (e.g., DraftKings/Consensus)
            ...
            # Fallback block to prevent empty displays if betting lines aren't published yet
            if not real_line_found or vegas_line is None or vegas_line == 0:
                ...
                over_odds, under_odds = -110, -110  # Standard pricing placeholder
            
            # --- CALCULATE IMPLIED PROBABILITY PER SIDE ---
...
if results_list:
    ...
    st.dataframe(
        board_df[[
            ...
            "XGBoost Proj", "Edge vs Book", "RECOMMENDATION"
        ]], 
    ...
    for idx, row in board_df.iterrows():
        with st.expander(f"{row['RECOMMENDATION']} - {row['Player']} ({row['Team']}) {row['Prop Category']} vs Line: {row['Book Line']}"):
            ...
            st.write(f"**Market Pricing Context:** Over: **{row['Over Odds']}** ({row['Implied Over %']}%) | Under: **{row['Under Odds']}** ({row['Implied Under %']}%)")
