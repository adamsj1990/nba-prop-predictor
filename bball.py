...
# Normalizer: Translates text strings from The Odds API to fit Balldontlie format
ODDS_API_TEAM_MAP = {
...
# --- THE ODDS API: FETCH MATCHING TARGET PROPERTIES ---
@st.cache_data(ttl=300)
def fetch_odds_api_props_by_teams(api_key, away_team, home_team):
    """Hits Odds API directory endpoint, matches the game, and grabs open player prop slates."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for event in events:
                api_away = ODDS_API_TEAM_MAP.get(event.get('away_team'), "")
                api_home = ODDS_API_TEAM_MAP.get(event.get('home_team'), "")
                
                if api_away == away_team and api_home == home_team:
                    event_id = event.get('id')
                    markets_csv = "player_points,player_rebounds,player_assists,player_threes"
                    props_url = f"https://api.the-odds-api.com/v4/sports/basketball_nba/events/{event_id}/odds?apiKey={api_key}&regions=us&markets={markets_csv}&oddsFormat=american"
                    props_resp = requests.get(props_url)
                    if props_resp.status_code == 200:
                        return props_resp.json().get('bookmakers', [])
        return []
    except:
        return []
...
def generate_sharp_recommendation(proj, line, stat):
    ...
    return "⚖️ HOLD (Line Efficient)", model_over_prob
...
def generate_rationale(name, stat, proj, line, model_prob, total_mod, def_mod, source):
    ...
    return f"XGBoost engine isolates a mathematical variance using [{source}] odds, assigning a {model_prob:.1f}% true probability toward the {direction}."
...
# --- SIDEBAR - SPORTSBOOK SELECTION DROPDOWN ---
...
# Strictly grab lines from Odds API payload
bookmakers_payload = fetch_odds_api_props_by_teams(THE_ODDS_API_KEY, away_team, home_team)
...
# --- FILTER ACTIVE ROSTERS (BALLDONTLIE HISTORY CADENCE) ---
...
if player_count == 0:
    ...
    for idx, (_, p_row) in enumerate(unique_players.iterrows()):
        ...
        for stat in stats_to_analyze:
            ...
            # --- THE ODDS API: MULTI-BOOK EXTRACTION ENGINE ---
            ...
            # Look up lines for the selected sportsbook
            ...
            # Fallback: Consensus line parsing from alternative bookmakers on the payload
                            ...
                            for outcome in market.get('outcomes', []):
                                ...
                                if p_name.lower().replace("-","").replace(" ","") in api_player.lower().replace("-","").replace(" ",""):
                                    ...
                                    if outcome.get('name', '').lower() == 'over':
                                        ...
                                        under_odds = outcome.get('price')
                                    line_source = f"⚠️ Consensus ({bookmaker.get('title')})"
                                    real_line_found = True
                                    break
