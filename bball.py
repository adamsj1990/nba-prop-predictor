...
# Sharp Matrix: Team Defensive Yield Coefficients vs Specific Stats
# Accounts for schematic defensive holes (< 1.0 = elite defense, > 1.0 = defensive leak)
...
def calculate_projected_pace(away_team, home_team, raw_df):
    """Sharp Analytic 1: Isolates rolling 30-day offensive possessions per matchup."""
...
def generate_sharp_recommendation(proj, line, implied_over_pct, stat):
    """Sharp Analytic 4: Probability curve variance optimization utilizing Z-scores."""
    edge = proj - line
...
def generate_rationale(name, stat, proj, line, model_prob, total_mod, spread_mod, def_mod, source):
    ...
    return f"XGBoost engine isolates a mathematical variance using [{source}] feeds, assigning a {model_prob:.1f}% true probability toward the {direction}{env_str}."
...
# --- SIDEBAR DROPDOWN FOR BOOK SPECIFIC REACTIVITY ---
...
# --- FILTER ACTIVE ROSTERS (STRICT 14-DAY RECENCY) ---
...
if player_count == 0:
    ...
    for idx, (_, p_row) in enumerate(unique_players.iterrows()):
        ...
        for stat in stats_to_analyze:
            ...
            # --- REAL-TIME BOOK ALIGNMENT PARSER ---
            real_line_found = False
            vegas_line, over_odds, under_odds = None, None, None
            line_source = f"🤖 Model Baseline"
            
            # Look up specific sportsbook selected by the user
            for api_prop in api_props_list:
                api_player = api_prop.get("player_name", "")
                api_type = PROP_MAP_CATEGORIES.get(api_prop.get("prop_type", ""), "")
                api_vendor = (api_prop.get("vendor") or api_prop.get("bookmaker") or "").lower().strip()
                
                if p_name.lower() in api_player.lower() and api_type == stat and clean_book_str in api_vendor:
            ...
            if not real_line_found:
                for api_prop in api_props_list:
                    api_player = api_prop.get("player_name", "")
                    api_type = PROP_MAP_CATEGORIES.get(api_prop.get("prop_type", ""), "")
                    api_vendor_alt = (api_prop.get("vendor") or api_prop.get("bookmaker") or "Market").strip()
                    ...
                    if p_name.lower() in api_player.lower() and api_type == stat:
                        ...
                        over_odds = market.get("over_odds", -110)
                        under_odds = market.get("under_odds", -110)
                        line_source = f"⚠️ Fallback ({api_vendor_alt})"
                        real_line_found = True
                        break
            ...
            # Sharp Analytic 3: Defensive Clash Multiplexing
            matchup_def_modifier = DEFENSIVE_YIELD_MULTIPLIER.get(stat, {}).get(opp_team, 1.0)
            
            # Sharp Feature Transformation: Train trees on volume deployment efficiency ratios
            ...
            rationale_text = generate_rationale(p_name, stat, prediction, vegas_line, model_calculated_prob, total_modifier, spread_modifier, matchup_def_modifier, line_source)
            
            results_list.append({
                "Player": p_name, "Team": p_team, "Prop Category": stat, "Book Line": vegas_line, "Line Source": line_source,
                "Over Odds": f"+{over_odds}" if over_odds > 0 else str(over_odds), "Implied Over %": round(implied_over_pct, 1),
                "Under Odds": f"+{under_odds}" if under_odds > 0 else str(under_odds), "Implied Under %": round(implied_under_pct, 1),
...
if results_list:
    ...
    st.dataframe(
        board_df[["Player", "Team", "Prop Category", "Book Line", "Line Source", "Over Odds", "Implied Over %", "Under Odds", "Implied Under %", "XGBoost Proj", "Model Win %", "RECOMMENDATION"]], 
