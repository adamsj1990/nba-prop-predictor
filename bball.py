...
def generate_rationale(name, stat, proj, line, model_prob, total_mod, spread_mod, def_mod, source):
    ...
    if def_mod > 1.02: env_impact.append("favorable structural scoring vulnerability")
    
    env_str = f" paired with {', '.join(env_impact)}" if env_impact else ""
    return f"XGBoost engine isolates a statistical variance using [{source}] data assigning a {model_prob:.1f}% true probability toward the {direction}{env_str}."
...
# --- DYNAMIC BOOKMAKER SELECTION DROPDOWN ---
...
if player_count == 0:
    ...
    for idx, (_, p_row) in enumerate(unique_players.iterrows()):
        ...
        for stat in stats_to_analyze:
            ...
            # --- OVERHAUL EXTRACTION VISIBILITY MATRIX ---
            line_source = f"🤖 Model Baseline"
            vegas_line = None
            over_odds = None
            under_odds = None
            
            # 1. Primary check: Search strictly for your selected oddsmaker
            ...
            # 2. Secondary Fallback: If chosen book is blank, pull general consensus and explicitly label it
            ...
            # 3. Tertiary Fallback: Algorithmic simulation if lines don't exist yet
...
if results_list:
    ...
    st.subheader(f"🔥 Top 20 Best Value Props Matrix: {selected_matchup}")
    st.dataframe(
        board_df[[
